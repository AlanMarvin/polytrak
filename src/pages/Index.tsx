...
const Index = () => {
  ...
  const features = [{
    ...
    description: 'Smart algorithms analyze trader performance',
    icon: Brain,
    useLogo: false
  }, {
    label: 'Optimized Settings',
    description: 'Auto-calculate ideal copy trading config for TheTradeFox',
    icon: null,
    useLogo: true
  }, {
    label: 'Personalized Strategy',
    description: 'Tailored to your risk & budget',
    icon: Sparkles,
    useLogo: false
  }];
  const previewSlides = [{
    ...
    icon: TrendingUp,
    preview: <div className="w-full h-32 flex items-end gap-1 px-4">
          {[20, 35, 25, 45, 40, 55, 50, 70, 65, 85, 75, 95].map((height, i) => <div key={i} className="flex-1 bg-gradient-to-t from-primary/50 to-primary rounded-t transition-all" style={{
        height: `${height}%`
      }} />)}
        </div>
  }, {
    ...
    icon: Settings,
    preview: <div className="space-y-3 w-full max-w-xs">
  ...
  return <Layout>
      ...
      <section className="relative overflow-hidden border-b border-border/50">
        ...
        <div className="container relative py-16 md:py-24">
          <div className="max-w-3xl mx-auto text-center space-y-6">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight flex flex-col items-center gap-2 md:gap-3">
              ...
              <span className="inline-flex items-center justify-center text-primary">
                <RotatingWord words={["recommended", "safer", "starting", "suggested", "practical"]} interval={2500} reducedMotionFallbackIndex={3} />
              </span>
            ...
            handleAnalyze();
              ...
              <div className="relative flex-1">
                ...
                <Input placeholder="Paste wallet address to analyze..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 h-12 bg-background/50 border-border/50 font-mono" />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-3xl mx-auto mt-12 md:mt-16">
            {features.map(feature => <div key={feature.label} className="text-center p-5 rounded-xl bg-card/50 border border-border/50 hover:border-primary/30 transition-colors">
                {feature.useLogo ? <img src={tradeFoxLogo} alt="TheTradeFox" className="h-10 w-auto mx-auto mb-3" /> : feature.icon && <feature.icon className="h-8 w-8 mx-auto mb-3 text-primary" />}
      ...
      <section className="container py-16 border-b border-border/50">
        ...
        <div className="max-w-4xl mx-auto px-8 md:px-12">
          ...
          loop: true
            ...
            <CarouselContent>
              {previewSlides.map((slide, index) => <CarouselItem key={slide.title} className="md:basis-full">
          ...
          <div className="flex justify-center gap-2 mt-6">
            {previewSlides.map((_, index) => <button key={index} onClick={() => carouselApi?.scrollTo(index)} className={`w-2 h-2 rounded-full transition-all ${currentSlide === index ? 'bg-primary w-6' : 'bg-muted-foreground/30 hover:bg-muted-foreground/50'}`} />)}
          </div>
        </div>
      </section>
      ...
      <section className="container py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
          {howItWorks.map((item, index) => <Card key={item.step} className="relative p-6 bg-card/50 border-border/50 hover:border-primary/30 transition-all hover:shadow-lg hover:shadow-primary/5">
              ...
              {index < howItWorks.length - 1 && <ArrowRight className="hidden lg:block absolute -right-3 top-1/2 -translate-y-1/2 h-6 w-6 text-muted-foreground/30" />}
            </Card>)}
        </div>
      </section>
      ...
      <section className="container py-16 border-t border-border/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          {aiFeatures.map(feature => <Card key={feature.title} className="p-6 bg-card/50 border-border/50 hover:border-accent/30 transition-colors">
      ...
      <section className="container pb-16">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-primary/20 via-primary/10 to-accent/20 border border-primary/20 p-8 md:p-12 bg-primary-foreground my-[90px] mx-[61px] px-0 py-0">
          ...
          <div className="relative text-center space-y-4 max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
              <Button size="lg" className="min-w-[180px]" onClick={() => document.querySelector('input')?.focus()}>
                <Search className="mr-2 h-4 w-4" />
                Start Analyzing
              </Button>
              <a href="https://thetradefox.com?ref=POLYTRAK" target="_blank" rel="noopener noreferrer">
